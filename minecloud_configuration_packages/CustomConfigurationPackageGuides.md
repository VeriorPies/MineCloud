...
3. Deploy MineCloud by following the typical deployment workflow. A placeholder Minecraft server should be spin up.
...
6. Prepare the `get_connection_count.sh` script:  
    ...
    - Once done, we can start preparing the config pack files
7. Edit EC2 initialization commands in `minecloud_configs/advanced_configs/custom-instance-init.ts`.  
   ...
   - Some useful APIs are:  
     ...
     - [InitPackage](https://docs.aws.amazon.com/cdk/api/v2/docs/aws-cdk-lib.aws_ec2.InitPackage.html): This can be used to install package require for the game server
...
12. Troubleshooting if something went wrong:  
    ...
    - If there's an issue with server start-up, follow the "Manually Start/Stop the Game Server" section in main [README.md](../README.md) for troubleshooting.
...
14. To do so:  
    ...
    - Edit `minecloud_configuration_packages/RELEASES.md` to add your configuration package download link.  
    - If it's a new game, also edit the main README to add it to the support list
