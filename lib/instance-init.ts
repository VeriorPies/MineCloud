...
function setUpShellScript(targetDir: string, targetFileName: string, localFilePath: string){
  return [
  ...
  return CloudFormationInit.fromConfigSets({
    ...
    configs: {
      ...
      setupServerUtilities: new InitConfig([
        ...
        ...setUpShellScript(MINECLOUD_SERVER_DIR, 'start_server.sh', 'minecloud_configs/server/start_server.sh'),
        ...setUpShellScript(MINECLOUD_SERVER_DIR, 'stop_server.sh', 'minecloud_configs/server/stop_server.sh'),
      ...
      setupDiscordMessaging: new InitConfig([
        ...setUpEnviromentVariable('DISCORD_WEB_HOOK', DISCORD_CHANNEL_WEB_HOOK),
        ...setUpShellScript(MINECLOUD_BASE_DIR, 'send_discord_message_to_webhook.sh', 'server_init_assets/send_discord_message_to_webhook.sh'),
      ]),
      setupMineCloudService: new InitConfig([
        ...setUpShellScript(MINECLOUD_BASE_DIR, 'start_service.sh', 'server_init_assets/start_service.sh'),
        InitFile.fromFileInline(
      ...
      setupBackupScripts: new InitConfig([
        ...
        ...setUpShellScript(MINECLOUD_BASE_DIR, 'server_backup.sh', 'server_init_assets/server_backup.sh'),
        ...setUpEnviromentVariable('MAX_BACKUP_COUNT', MAX_BACKUP_COUNT.toString()),
        ...setUpShellScript(MINECLOUD_BASE_DIR, 'server_manual_backup.sh', 'server_init_assets/server_manual_backup.sh'),
        ...setUpShellScript(MINECLOUD_BASE_DIR, 'auto_backup_checker.sh', 'server_init_assets/auto_backup_checker.sh'),
        ...setUpEnviromentVariable('BACKUP_INTERVAL', BACKUP_INTERVAL_IN_SECONDS.toString()),
      ]),
      setupAutoShutdown: new InitConfig([
        // Setup custom logic for checking how many current connection
        ...setUpShellScript(MINECLOUD_BASE_DIR, 'get_connection_count.sh', 'minecloud_configs/advanced_configs/get_connection_count.sh'),
        ...setUpShellScript(MINECLOUD_BASE_DIR, 'check_user_conn.sh', 'server_init_assets/check_user_conn.sh'),
      ...
      setupGetLatestBackupScript: new InitConfig([
        ...setUpShellScript(MINECLOUD_BASE_DIR, 'get_latest_server_backup.sh', 'server_init_assets/get_latest_server_backup.sh'),
      ]),
