...
export const INTANCE_INIT_CONFIG = CloudFormationInit.fromConfigSets(
    {
      configSets: {
        default: [
          "yumPreinstall",
      ...
      configs: {
        ...
        setupDiscordMessaging: new InitConfig([
          InitCommand.shellCommand( `echo 'DISCORD_WEB_HOOK=${DISCORD_CHANNEL_WEB_HOOK}' >> /etc/environment`),
        ...
        setupBackupScripts: new InitConfig([
          ...
          InitCommand.shellCommand(`echo 'MAX_BACKUP_COUNT=${MAX_BACKUP_COUNT}' >> /etc/environment`),

          InitFile.fromFileInline(`${MINECRAFT_BASE_DIR}/server_manual_backup.sh`,'server_init_assets/server_manual_backup.sh'),
          InitCommand.shellCommand(`sudo chmod +x server_manual_backup.sh`, {cwd: MINECRAFT_BASE_DIR}),
          
          InitFile.fromFileInline(`${MINECRAFT_BASE_DIR}/auto_backup_checker.sh`,'server_init_assets/auto_backup_checker.sh'),
          InitCommand.shellCommand(`sudo chmod +x auto_backup_checker.sh`, {cwd: MINECRAFT_BASE_DIR}),
          InitCommand.shellCommand(`echo 'BACKUP_INTERVAL=${BACKUP_INTERVAL_IN_SECONDS}' >> /etc/environment`),
        ]),
